...
do
	...
	function ColorSlider:willUnmount()
...
local ColorPickerInside do
	local BorderedFrame = require(Foundation.BorderedFrame)
	local PreciseButton = require(Foundation.PreciseButton)
	local VerticalList = require(Foundation.VerticalList)

	ColorPickerInside = Roact.PureComponent:extend("ColorPickerInside")

	function ColorPickerInside:init()
		self.state = {
			valuePressed = false,
			chromaPressed = false
		}
	...
	ColorPickerInside.defaultProps = {
		modalIndex = 0
	}

	function ColorPickerInside:render()
		return withTheme(
			function(theme)
				local props = self.props
	...
	function ColorPickerInside:willUnmount()
...
do
	...
	ColorPickerModal = Roact.PureComponent:extend("ColorPickerModal")
	function ColorPickerModal:init()
		local props = self.props
		local color = props.color
		self.state = {
			initial_color = color
	...
	function ColorPickerModal:render()
		return withTheme(
			function(theme)
				return withModal(
						...
						return Roact.createElement(
							...
							},
							{
								...
								Window = Roact.createElement(
									...
									{
										ColorPickerInside = Roact.createElement(
											ColorPickerInside,
											{
												h = 0,
												s = 0,
...
local ThemedColorSelector = Roact.PureComponent:extend("ThemedColorSelector")
...
local function formatRGB255(r, g, b)
	return string.format("%d, %d, %d", r, g, b)
end

function ThemedColorSelector:init()
	self.newTextValidateCallback = function(s)
		return s:match("^[-%d, ]*$") ~= nil
	end

	self.onFocusLost = function(s)
		...
		if self.props.onColorChanged then
			...
			self.props.onColorChanged(color)
		end
		return formatRGB255(r, g, b)
	end

	self.textFormatCallback = function(s)
		local split = string.split(s, ",")
		local r, g, b = tonumber(split[1]), tonumber(split[2]), tonumber(split[3])
		local color = Color3.fromRGB(r, g, b)
		local brickColor = BrickColor.new(color)
		local name = brickColor.Name
		return string.format("[%d, %d, %d] (%s)", r, g, b, name)
	end
end

ThemedColorSelector.defaultProps = {
	modalIndex = 0,
	Position = UDim2.new(0, 0, 0, 0),
	Size = UDim2.new(1, 0, 0, Constants.INPUT_FIELD_BOX_HEIGHT),
	AnchorPoint = Vector2.new(0, 0),
...
function ThemedColorSelector:init()
	...
	self.onClick = function()
		if not self.state.colorPickerOpen then
			self:setState(
				{
...
function ThemedColorSelector:render()
	local props = self.props
	local color = props.color
	local Position = props.Position
	local Size = props.Size
	...
	local colorStr = formatRGB255(r, g, b)
		...
		function(theme)
			return withModal(
				function()
					...
					local buttonTextColor = not disabled and buttonTheme.textColor.Default or buttonTheme.textColor.Disabled
					...
					return Roact.createElement(
						...
						{
							ColorPicker = self.state.colorPickerOpen and Roact.createElement(
								ColorPickerModal,
								{
									color = color
								}
